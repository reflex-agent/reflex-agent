...
const BUILTIN: Skill[] = [
  ...
  },
  {
    ...
    description:
      "Chat-driven intake for a new Space. Captures goals to project memory and proposes utilities, research, and widgets to set up.",
    author: "builtin",
    instructions: [
      "## Skill: space-onboarding",
      "",
      "You are running the first-conversation intake for a freshly-created Space (Reflex project). Goal of this conversation: understand WHY the user opened this Space well enough to propose 3-7 concrete next actions they can approve from the dashboard.",
      "",
      "### Tone",
      ...
      "### Flow",
      "",
      "Turn 1 (now): Open with one welcoming sentence and ONE focused question — what's this Space for, in one line? Don't preface with multi-paragraph rules.",
      "",
      "Turns 2-4: Drill down. Aim for these axes, but order/wording adapts to what they said:",
      ...
      "",
      "Turn ~4-6 (closing): Once you have a clear picture, switch to proposing.",
      "  1. Write any durable facts the user mentioned into project memory via `<<reflex:memory>>{scope:\"project\",...}`. Especially:",
      "     - **GOALS** — concrete outcomes for this Space.",
      "     - **PERSONA** — what this Space IS (e.g. \"PhD thesis on attention research\", \"daily fitness log\").",
      "     - **INTERESTS** — sub-topics they want explored.",
      "  2. If you learned something about the user's identity that transcends this Space (\"I'm a vegetarian\", \"I have a 3yo daughter\") → write to scope=global instead.",
      "  3. Emit 3-7 `<<reflex:suggestion>>` markers — one per concrete next action you'd recommend.",
      "  4. Tell the user, in 2 lines: \"I've put suggestions on the dashboard — approve the ones you want and ignore the rest.\" STOP. Do NOT execute them yourself.",
      "",
      ...
      "",
      "Examples by kind:",
      "- **utility** → `{kind:\"utility\",title:\"Install Learn-anything\",description:\"You said you want a structured curriculum — this utility builds one.\",prompt:\"/util install learn-anything\"}`",
      "- **research** → `{kind:\"research\",title:\"Survey resting-HRV trackers\",description:\"You want to track HRV; let's compare options first.\",prompt:\"/research best HRV tracking apps for everyday use, with citations\"}`",
      "- **widget** → `{kind:\"widget\",title:\"Weekly run distance KPI\",description:\"You want a single number to check on Sunday.\",prompt:\"/widget weekly running distance KPI, refresh weekly\"}`",
      "- **goal** → `{kind:\"goal\",title:\"Run a half-marathon by October\",description:\"Anchor goal you mentioned — keep it active.\",prompt:\"/goal run a half-marathon by 2026-10-15; weekly check-ins on training volume\"}`",
      "- **skill** → `{kind:\"skill\",title:\"Use deep-research on study questions\",description:\"For literature reviews you'll want citation discipline.\",prompt:\"/skill deep-research\"}`",
      "",
      "### Rules",
      "",
      "- Don't propose more than 7 suggestions. Quality > volume.",
      "- Don't propose generic utilities (e.g. \"unit-converter\") unless the user clearly hinted they need one.",
      "- Cite what they said when describing a suggestion (\"Because you mentioned X…\"). Makes it easy to approve/reject.",
      "- Never start the topic for them. The dashboard's approve button does that — your job ends after the markers are out.",
      "- If the user wraps up early (\"that's enough\", \"let's just see what you suggest\") — go straight to closing turn, propose with whatever signal you have.",
    ].join("\n"),
  },
  {
    id: "memory-rollup",
